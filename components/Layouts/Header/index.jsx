...
const solutions = [
  ...
  { name: 'Personal Information', description: "Your customers' data will be safe and secure.", href: '/Information', icon: UserIcon },
...
const resources = [
  ...
  {
    name: 'Contact',
    description: 'You will see all my social media information to easily contact me.',
    href: 'mailto:marco.deguzman.ico@gmail.com',
    icon: PhoneIcon,
  },
  {
  ...
  { name: 'Security', description: 'Understand how I take your privacy seriously.', href: '/', icon: ShieldCheckIcon },
]
...
export default function index({activePage}) {
  ...
  return (
    ...
    <Popover className="relative dark:bg-black-400">
      ...
      className={`${
        enabled ? ' bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 ' : 'bg-gray-200'
      } relative inline-flex h-6 w-11 items-center rounded-full`}
    >
      
      <span
        className={`${
          enabled ? 'translate-x-6 ring-1 ring-white' : 'translate-x-1 bg-white'
        } inline-block h-4 w-4 transform rounded-full p-1  transition`}
      >{enabled?<MoonIcon/>:<SunIcon/>}</span>
    </Switch>
    </div>
              ...
              <p className='text-white px-2.5 py-2 rounded-full bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500'>MI</p>
                ...
                <div className="-mr-2">
                  <Popover.Button className="inline-flex items-center justify-center dark:bg-gray-900 rounded-md bg-white p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500">
                    ...
                    <XMarkIcon className="h-6 w-6 dark:bg-gray-900" aria-hidden="true" />
                  </Popover.Button>
                </div>
              </div>
              <div className="mt-6">
